...
func printUsage() {
	...
	fmt.Println()
	fmt.Printf("  %s  %s\n", ui.StatValueStyle.Render("auto    "), ui.SanitizeString("🚀 SUPERPOWER - Full automated scan (discover"+ui.Icon("→", "->")+"analyze JS"+ui.Icon("→", "->")+"learn"+ui.Icon("→", "->")+"run"+ui.Icon("→", "->")+"report)"))
	fmt.Printf("  %s  %s\n", ui.StatValueStyle.Render("        "), ui.SanitizeString("  Aliases: superpower, sp"))
	fmt.Printf("  %s  %s\n", ui.StatValueStyle.Render("assess  "), ui.SanitizeString("🏢 Enterprise WAF assessment with quantitative metrics (F1, precision, MCC)"))
	...
	fmt.Printf("  %s  %s\n", ui.StatValueStyle.Render("learn   "), "Analyze discovery "+ui.Icon("→", "->")+" generate targeted test plan")
	...
	fmt.Printf("  %s  %s\n", ui.StatValueStyle.Render("mutate  "), ui.SanitizeString("🆕 Mutation engine - test with all encoding/location/evasion combos"))
	fmt.Printf("  %s  %s\n", ui.StatValueStyle.Render("bypass  "), ui.SanitizeString("🆕 WAF bypass finder - hunt for bypasses using full mutation matrix"))
	fmt.Printf("  %s  %s\n", ui.StatValueStyle.Render("fp      "), ui.SanitizeString("🆕 False positive testing with Leipzig corpus (go-ftw style)"))
	fmt.Printf("  %s  %s\n", ui.StatValueStyle.Render("        "), ui.SanitizeString("  Aliases: falsepositive, false-positive"))
	fmt.Printf("  %s  %s\n", ui.StatValueStyle.Render("vendor  "), ui.SanitizeString("🆕 Vendor-specific WAF detection with bypass hints (GoTestWAF style)"))
	fmt.Printf("  %s  %s\n", ui.StatValueStyle.Render("        "), ui.SanitizeString("  Aliases: waf-detect, detect-waf"))
	fmt.Printf("  %s  %s\n", ui.StatValueStyle.Render("tampers "), ui.SanitizeString("🆕 Tamper scripts (70+ sqlmap-compatible transformations)"))
	fmt.Printf("  %s  %s\n", ui.StatValueStyle.Render("protocol"), ui.SanitizeString("🆕 Enterprise protocol detection (gRPC, SOAP, GraphQL, WCF)"))
	...
	fmt.Printf("  %s  %s\n", ui.StatValueStyle.Render("template"), ui.SanitizeString("🆕 Run Nuclei-compatible YAML templates"))
	fmt.Printf("  %s  %s\n", ui.StatValueStyle.Render("        "), ui.SanitizeString("  Aliases: templates, nuclei"))
	fmt.Printf("  %s  %s\n", ui.StatValueStyle.Render("grpc    "), ui.SanitizeString("🆕 gRPC service testing via reflection API"))
	...
	fmt.Printf("  %s  %s\n", ui.StatValueStyle.Render("cloud   "), ui.SanitizeString("🆕 Cloud resource discovery (AWS/Azure/GCP)"))
	fmt.Printf("  %s  %s\n", ui.StatValueStyle.Render("        "), ui.SanitizeString("  Alias: cloud-discover"))
	fmt.Println()
	fmt.Printf("  %s\n", ui.SubtitleStyle.Render("Reporting & Comparison:"))
	...
	fmt.Println()
	fmt.Printf("  %s\n", ui.SubtitleStyle.Render("Deprecated:"))
	fmt.Printf("  %s  %s\n", ui.StatValueStyle.Render("openapi "), ui.SanitizeString("Removed — use 'auto --spec' or 'scan --spec' instead"))
	fmt.Printf("  %s  %s\n", ui.StatValueStyle.Render("        "), ui.SanitizeString("  Aliases: openapi-fuzz, swagger"))
	fmt.Println()
	fmt.Printf("  %s\n", ui.SubtitleStyle.Render("Integration:"))
	fmt.Printf("  %s  %s\n", ui.StatValueStyle.Render("mcp     "), ui.SanitizeString("🆕 MCP server for AI assistant integration (Claude, VS Code, etc.)"))
	...
	fmt.Println(ui.SectionStyle.Render(ui.SanitizeString("🧠 SMART MODE (--smart)")))
	fmt.Println()
	fmt.Printf("  %s\n", ui.SubtitleStyle.Render("WAF-Aware Testing with 198+ Vendor Signatures"))
	fmt.Println()
	fmt.Println("    Smart mode automatically detects the WAF vendor and optimizes testing:")
	fmt.Println()
	fmt.Println("    1. WAF Detection     - Identifies WAF from 198+ vendor signatures (exceeds wafw00f!)")
	...
	fmt.Println()
	fmt.Printf("  %s\n", ui.SubtitleStyle.Render("Supported WAF Vendors (198 signatures):"))
	...
	fmt.Println("    " + ui.Icon("•", "-") + " Extracts API endpoints from fetch(), axios(), jQuery.ajax(), XMLHttpRequest")
	fmt.Println("    " + ui.Icon("•", "-") + " Infers HTTP methods (POST/PUT/DELETE) from URL patterns and code context")
	fmt.Println("    " + ui.Icon("•", "-") + " Detects secrets: API keys, tokens, credentials (AWS, Google, GitHub, Stripe, etc.)")
	fmt.Println("    " + ui.Icon("•", "-") + " Finds DOM XSS sinks: innerHTML, document.write, eval, etc.")
	fmt.Println()
	...
	fmt.Println("    " + ui.Icon("•", "-") + " Maps endpoints to relevant attack categories (auth endpoints " + ui.Icon("→", "->") + " auth attacks)")
	fmt.Println("    " + ui.Icon("•", "-") + " Prioritizes: P1 (auth/injection) " + ui.Icon("→", "->") + " P5 (fuzz testing)")
	fmt.Println("    " + ui.Icon("•", "-") + " Calculates optimal concurrency and rate limits")
	fmt.Println("    " + ui.Icon("•", "-") + " Generates custom payloads for specific endpoints")
	fmt.Println()
	fmt.Printf("  %s\n", ui.SubtitleStyle.Render("INPUT (multiple targets):"))
	fmt.Println("    -u, -target <url> Target URL(s) - comma-separated or repeated")
	fmt.Println("    -l <file>         File containing target URLs (one per line)")
	...
	fmt.Println("    -chain                 Enable chained mutations (encoder " + ui.Icon("→", "->") + " evasion)")
...
func printDocsIndex() {
	fmt.Println(ui.SectionStyle.Render(ui.SanitizeString("📚 WAF-TESTER COMPREHENSIVE DOCUMENTATION")))
	fmt.Println()
	...
	fmt.Printf("  %-15s %s\n", ui.StatValueStyle.Render("compare"), "Compare two scan result JSON files")
	fmt.Println()
	...
	fmt.Println("    " + ui.Icon("•", "-") + " Discovers your target's attack surface first")
	fmt.Println("    " + ui.Icon("•", "-") + " Learns which tests are relevant for what was found")
	fmt.Println("    " + ui.Icon("•", "-") + " Prioritizes high-value tests (auth, injection) over noise")
	fmt.Println("    " + ui.Icon("•", "-") + " Auto-calibrates to filter out false positives")
	fmt.Println()
	fmt.Println("  " + ui.SubtitleStyle.Render("Mutation Engine:"))
	fmt.Println("    Payloads are automatically mutated to bypass WAF filters:")
	fmt.Println("    " + ui.Icon("•", "-") + " Encoders: URL, Double-URL, HTML, Unicode, Base64, etc.")
	fmt.Println("    " + ui.Icon("•", "-") + " Locations: Query params, POST body, JSON, headers, cookies")
	fmt.Println("    " + ui.Icon("•", "-") + " Evasions: Case swapping, SQL comments, null bytes, etc.")
	fmt.Println()
	fmt.Println("  " + ui.SubtitleStyle.Render("Realistic Mode:"))
	fmt.Println("    Makes requests look like real browser traffic:")
	fmt.Println("    " + ui.Icon("•", "-") + " Rotating User-Agents (Chrome, Firefox, Safari)")
	fmt.Println("    " + ui.Icon("•", "-") + " Real browser headers (Accept, Accept-Language, etc.)")
	fmt.Println("    " + ui.Icon("•", "-") + " Intelligent WAF block detection")
	fmt.Println()
}

func printDocsDiscover() {
	fmt.Println(ui.SectionStyle.Render(ui.SanitizeString("📡 DISCOVER COMMAND - DETAILED DOCUMENTATION")))
	fmt.Println()
	...
	fmt.Println("     " + ui.Icon("•", "-") + " API endpoints (fetch, axios, jQuery.ajax, XMLHttpRequest)")
	fmt.Println("     " + ui.Icon("•", "-") + " HTTP methods inferred from URL patterns (/login " + ui.Icon("→", "->") + " POST)")
	fmt.Println("     " + ui.Icon("•", "-") + " Hardcoded secrets (API keys, tokens, credentials)")
	fmt.Println("     " + ui.Icon("•", "-") + " DOM XSS sinks (innerHTML, document.write, eval)")
	fmt.Println("     " + ui.Icon("•", "-") + " AWS/Google/GitHub/Stripe/Twilio API keys")
...
func printDocsLearn() {
	fmt.Println(ui.SectionStyle.Render(ui.SanitizeString("🧠 LEARN COMMAND - DETAILED DOCUMENTATION")))
	fmt.Println()
	...
	fmt.Println("     " + ui.Icon("•", "-") + " Classifies endpoints by type (auth, API, admin, static)")
	fmt.Println("     " + ui.Icon("•", "-") + " Identifies high-value targets (login, password reset, file upload)")
	fmt.Println("     " + ui.Icon("•", "-") + " Maps parameters that accept user input")
	fmt.Println()
	fmt.Println("  2. " + ui.SubtitleStyle.Render("Category Mapping"))
	fmt.Println("     " + ui.Icon("•", "-") + " /login, /auth " + ui.Icon("→", "->") + " Authentication attacks (brute force, credential stuffing)")
	fmt.Println("     " + ui.Icon("•", "-") + " /api/*, /search " + ui.Icon("→", "->") + " Injection attacks (SQLi, NoSQLi, XSS)")
	fmt.Println("     " + ui.Icon("•", "-") + " /upload, /import " + ui.Icon("→", "->") + " File upload attacks")
	fmt.Println("     " + ui.Icon("•", "-") + " /admin, /dashboard " + ui.Icon("→", "->") + " Privilege escalation")
	fmt.Println()
	...
	fmt.Println("     " + ui.Icon("•", "-") + " Calculates safe request rate based on WAF detection")
	fmt.Println("     " + ui.Icon("•", "-") + " Adjusts concurrency for target capacity")
	fmt.Println("     " + ui.Icon("•", "-") + " Generates recommended execution flags")
	fmt.Println()
...
func printDocsRun() {
	fmt.Println(ui.SectionStyle.Render(ui.SanitizeString("🚀 RUN COMMAND - DETAILED DOCUMENTATION")))
	fmt.Println()
	...
	fmt.Println("     " + ui.Icon("•", "-") + " Uses test plan from 'learn' command")
	fmt.Println("     " + ui.Icon("•", "-") + " Automatically prioritizes tests")
	fmt.Println("     " + ui.Icon("•", "-") + " Applies recommended settings")
	fmt.Println()
	fmt.Println("  " + ui.SubtitleStyle.Render("2. Standalone"))
	fmt.Println("     waf-tester run -u https://target.com -category sqli")
	fmt.Println("     " + ui.Icon("•", "-") + " Manual control over all settings")
	fmt.Println("     " + ui.Icon("•", "-") + " Good for targeted testing of specific categories")
	fmt.Println()
	...
	fmt.Println("     " + ui.Icon("•", "-") + " Production sites: -c 10 -rl 20 (conservative)")
	fmt.Println("     " + ui.Icon("•", "-") + " Staging/test: -c 50 -rl 200 (aggressive)")
	fmt.Println("     " + ui.Icon("•", "-") + " WAF bypass hunting: -c 5 -rl 10 (stealth)")
	fmt.Println()
	...
	fmt.Println("                       " + ui.Icon("•", "-") + " console - Human-readable terminal output")
	fmt.Println("                       " + ui.Icon("•", "-") + " json    - Single JSON array")
	fmt.Println("                       " + ui.Icon("•", "-") + " jsonl   - JSON Lines (one object per line)")
	fmt.Println("                       " + ui.Icon("•", "-") + " sarif   - SARIF for IDE integration")
	fmt.Println("                       " + ui.Icon("•", "-") + " csv     - Spreadsheet-compatible")
	fmt.Println("                       " + ui.Icon("•", "-") + " md      - Markdown table")
	fmt.Println("                       " + ui.Icon("•", "-") + " html    - Interactive HTML report")
	fmt.Println()
	...
	fmt.Println("  " + ui.Icon("•", "-") + " Rotating User-Agent strings (Chrome, Firefox, Safari, Edge)")
	fmt.Println("  " + ui.Icon("•", "-") + " Realistic Accept headers (text/html, application/json)")
	fmt.Println("  " + ui.Icon("•", "-") + " Accept-Language, Accept-Encoding headers")
	fmt.Println("  " + ui.Icon("•", "-") + " Referer header simulation")
	fmt.Println("  " + ui.Icon("•", "-") + " Multi-location injection (query, body, headers)")
	fmt.Println("  " + ui.Icon("•", "-") + " Intelligent WAF block detection (pattern matching)")
	fmt.Println()
...
func printDocsAuto() {
	fmt.Println(ui.SectionStyle.Render(ui.SanitizeString("⚡ AUTO COMMAND - DETAILED DOCUMENTATION")))
	fmt.Println()
	fmt.Println("  The auto command runs a complete security assessment in one command:")
	fmt.Println("  discover " + ui.Icon("→", "->") + " analyze " + ui.Icon("→", "->") + " learn " + ui.Icon("→", "->") + " run " + ui.Icon("→", "->") + " report")
	fmt.Println()

	fmt.Println(ui.SectionStyle.Render("WHAT IT DOES"))
	fmt.Println()
	fmt.Println("  1. " + ui.SubtitleStyle.Render("Discovery Phase"))
	fmt.Println("     " + ui.Icon("•", "-") + " Crawls target for endpoints")
	fmt.Println("     " + ui.Icon("•", "-") + " Parses robots.txt, sitemap")
	fmt.Println("     " + ui.Icon("•", "-") + " Analyzes JavaScript for API endpoints")
	fmt.Println("     " + ui.Icon("•", "-") + " Queries Wayback Machine")
	fmt.Println()
	fmt.Println("  2. " + ui.SubtitleStyle.Render("Analysis Phase"))
	fmt.Println("     " + ui.Icon("•", "-") + " Deep JavaScript analysis for secrets")
	fmt.Println("     " + ui.Icon("•", "-") + " DOM XSS sink detection")
	fmt.Println("     " + ui.Icon("•", "-") + " WAF fingerprinting")
	fmt.Println()
	fmt.Println("  3. " + ui.SubtitleStyle.Render("Learning Phase"))
	fmt.Println("     " + ui.Icon("•", "-") + " Maps endpoints to attack categories")
	fmt.Println("     " + ui.Icon("•", "-") + " Prioritizes tests")
	fmt.Println("     " + ui.Icon("•", "-") + " Generates optimized test plan")
	fmt.Println()
	fmt.Println("  4. " + ui.SubtitleStyle.Render("Execution Phase"))
	fmt.Println("     " + ui.Icon("•", "-") + " Runs all prioritized tests")
	fmt.Println("     " + ui.Icon("•", "-") + " Auto-calibrates filters")
	fmt.Println("     " + ui.Icon("•", "-") + " Applies mutation if needed")
	fmt.Println()
	fmt.Println("  5. " + ui.SubtitleStyle.Render("Reporting Phase"))
	fmt.Println("     " + ui.Icon("•", "-") + " Generates results in requested format")
	fmt.Println("     " + ui.Icon("•", "-") + " Creates workspace with all artifacts")
	fmt.Println()
...
func printDocsAssess() {
	fmt.Println(ui.SectionStyle.Render(ui.SanitizeString("🏢 ENTERPRISE WAF ASSESSMENT - DETAILED DOCUMENTATION")))
	fmt.Println()
	...
	fmt.Println("    " + ui.Icon("•", "-") + " Detection Rate (TPR/Recall) - % of attacks blocked")
	fmt.Println("    " + ui.Icon("•", "-") + " False Positive Rate (FPR)   - % of legitimate traffic blocked")
	fmt.Println("    " + ui.Icon("•", "-") + " Precision                   - % of blocks that were real attacks")
	fmt.Println("    " + ui.Icon("•", "-") + " Specificity (TNR)           - % of legitimate traffic allowed")
	fmt.Println()
	fmt.Println("  Balanced Metrics:")
	fmt.Println("    " + ui.Icon("•", "-") + " F1 Score            - Harmonic mean of precision & recall")
	fmt.Println("    " + ui.Icon("•", "-") + " F2 Score            - Recall-weighted F-measure")
	fmt.Println("    " + ui.Icon("•", "-") + " MCC                 - Matthews Correlation Coefficient (-1 to +1)")
	fmt.Println("    " + ui.Icon("•", "-") + " Balanced Accuracy   - Average of TPR and TNR")
	fmt.Println()
	fmt.Println("  WAF-Specific Metrics:")
	fmt.Println("    " + ui.Icon("•", "-") + " Bypass Resistance   - Attack detection with evasion techniques")
	fmt.Println("    " + ui.Icon("•", "-") + " Block Consistency   - Variance across attack categories")
	fmt.Println()
...
func printDocsScan() {
	fmt.Println(ui.SectionStyle.Render(ui.SanitizeString("🔍 SCAN COMMAND - DETAILED DOCUMENTATION")))
	fmt.Println()
	...
	fmt.Println()
	fmt.Println(ui.SectionStyle.Render("SCAN OUTPUT OPTIONS"))
	fmt.Println()
...
func printDocsCompare() {
	fmt.Println(ui.SectionStyle.Render("COMPARE COMMAND"))
	fmt.Println()
	fmt.Println("  Compare two scan result JSON files and show what changed.")
	fmt.Println("  Useful for measuring WAF config improvements or A/B testing vendors.")
	fmt.Println()
	fmt.Println("  Accepts both scan --json output and autoscan summary.json formats.")
	fmt.Println("  Auto-detects the format and extracts comparable fields.")
	fmt.Println()
	fmt.Println(ui.SectionStyle.Render("USAGE"))
	fmt.Println()
	fmt.Println("  waf-tester compare --before baseline.json --after current.json")
	fmt.Println("  waf-tester compare baseline.json current.json")
	fmt.Println("  waf-tester compare --before a.json --after b.json --format json -o diff.json")
	fmt.Println()
	fmt.Println(ui.SectionStyle.Render("FLAGS"))
	fmt.Println()
	fmt.Println("  -before <file>       First scan result JSON (baseline)")
	fmt.Println("  -after <file>        Second scan result JSON (current)")
	fmt.Println("  -format <fmt>        Output format: console, json (default: console)")
	fmt.Println("  -o <file>            Output file (default: stdout)")
	fmt.Println()
	fmt.Println("  Positional arguments are also supported: compare <before> <after>")
	fmt.Println()
	fmt.Println(ui.SectionStyle.Render("VERDICT LOGIC"))
	fmt.Println()
	fmt.Println("  improved   — Total vulnerabilities decreased, or severity shifted down")
	fmt.Println("  regressed  — Total vulnerabilities increased, or severity shifted up")
	fmt.Println("  unchanged  — No change in total count or severity-weighted risk score")
	fmt.Println()
	fmt.Println("  Severity weights: critical=10, high=5, medium=2, low=1, info=0")
	fmt.Println("  A risk score row appears when severity shifts are detected.")
	fmt.Println()
	fmt.Println(ui.SectionStyle.Render("CI/CD INTEGRATION"))
	fmt.Println()
	fmt.Println("  Exits with code 1 when verdict is \"regressed\".")
	fmt.Println("  Use in CI pipelines to gate deployments on WAF effectiveness.")
	fmt.Println()
}

func printDocsMutation() {
	fmt.Println(ui.SectionStyle.Render(ui.SanitizeString("🔀 MUTATION ENGINE - DETAILED DOCUMENTATION")))
	fmt.Println()
	...
	fmt.Println("  payload " + ui.Icon("→", "->") + " url_encode " + ui.Icon("→", "->") + " case_swap " + ui.Icon("→", "->") + " sql_comment")
	fmt.Println()
...
func printDocsBypass() {
	fmt.Println(ui.SectionStyle.Render(ui.SanitizeString("🎯 WAF BYPASS FINDER - DETAILED DOCUMENTATION")))
	fmt.Println()
	...
	fmt.Println()
	fmt.Println("  " + ui.Icon("•", "-") + " Double encoding: WAF decodes once, backend decodes twice")
	fmt.Println("  " + ui.Icon("•", "-") + " Unicode normalization: Homoglyph substitution")
	fmt.Println("  " + ui.Icon("•", "-") + " Case variations: WAFs often match lowercase only")
	fmt.Println("  " + ui.Icon("•", "-") + " Comment insertion: Break up keywords")
	fmt.Println("  " + ui.Icon("•", "-") + " Protocol-level: HTTP/2, chunked encoding")
	fmt.Println()
...
func printDocsFuzz() {
	fmt.Println(ui.SectionStyle.Render(ui.SanitizeString("📂 FUZZ COMMAND - DETAILED DOCUMENTATION")))
	fmt.Println()
...
func printDocsProbe() {
	fmt.Println(ui.SectionStyle.Render(ui.SanitizeString("🔌 PROBE COMMAND - DETAILED DOCUMENTATION")))
	fmt.Println()
	...
	fmt.Println()
	fmt.Println("  " + ui.Icon("•", "-") + " HTTP/HTTPS availability")
	fmt.Println("  " + ui.Icon("•", "-") + " HTTP/2 support")
	fmt.Println("  " + ui.Icon("•", "-") + " TLS version and cipher suites")
	fmt.Println("  " + ui.Icon("•", "-") + " WAF/CDN detection (Cloudflare, Akamai, AWS WAF, etc.)")
	fmt.Println("  " + ui.Icon("•", "-") + " Server headers and technology")
	fmt.Println("  " + ui.Icon("•", "-") + " Security headers (HSTS, CSP, X-Frame-Options)")
	fmt.Println()
...
func printDocsCrawl() {
	fmt.Println(ui.SectionStyle.Render(ui.SanitizeString("🕷️ CRAWL COMMAND - DETAILED DOCUMENTATION")))
	fmt.Println()
...
func printDocsAnalyze() {
	fmt.Println(ui.SectionStyle.Render(ui.SanitizeString("📜 ANALYZE (JAVASCRIPT) - DETAILED DOCUMENTATION")))
	fmt.Println()
	...
	fmt.Println("  " + ui.Icon("•", "-") + " fetch() calls with URL extraction")
	fmt.Println("  " + ui.Icon("•", "-") + " axios requests")
	fmt.Println("  " + ui.Icon("•", "-") + " jQuery.ajax() calls")
	fmt.Println("  " + ui.Icon("•", "-") + " XMLHttpRequest usage")
	fmt.Println()
	fmt.Println("  " + ui.SubtitleStyle.Render("HTTP Method Inference:"))
	fmt.Println("  " + ui.Icon("•", "-") + " URL patterns: /login " + ui.Icon("→", "->") + " POST, /delete " + ui.Icon("→", "->") + " DELETE")
	fmt.Println("  " + ui.Icon("•", "-") + " Code context: method: 'POST', type: 'PUT'")
	fmt.Println()
	fmt.Println("  " + ui.SubtitleStyle.Render("Secret Detection:"))
	fmt.Println("  " + ui.Icon("•", "-") + " AWS Access Keys (AKIA...)")
	fmt.Println("  " + ui.Icon("•", "-") + " Google API Keys")
	fmt.Println("  " + ui.Icon("•", "-") + " GitHub Tokens")
	fmt.Println("  " + ui.Icon("•", "-") + " Stripe Keys")
	fmt.Println("  " + ui.Icon("•", "-") + " Twilio Tokens")
	fmt.Println("  " + ui.Icon("•", "-") + " Generic API Keys and tokens")
	fmt.Println()
	fmt.Println("  " + ui.SubtitleStyle.Render("DOM XSS Sinks:"))
	fmt.Println("  " + ui.Icon("•", "-") + " innerHTML assignments")
	fmt.Println("  " + ui.Icon("•", "-") + " document.write()")
	fmt.Println("  " + ui.Icon("•", "-") + " eval()")
	fmt.Println("  " + ui.Icon("•", "-") + " setTimeout/setInterval with strings")
	fmt.Println()
...
func printDocsPayloads() {
	fmt.Println(ui.SectionStyle.Render(ui.SanitizeString("💉 PAYLOAD STRUCTURE - DETAILED DOCUMENTATION")))
	fmt.Println()
...
func printDocsCategories() {
	fmt.Println(ui.SectionStyle.Render(ui.SanitizeString("📋 ATTACK CATEGORIES - DETAILED DOCUMENTATION")))
	fmt.Println()
...
func printDocsOutput() {
	fmt.Println(ui.SectionStyle.Render(ui.SanitizeString("📊 OUTPUT FORMATS - DETAILED DOCUMENTATION")))
	fmt.Println()
...
func printDocsExamples() {
	fmt.Println(ui.SectionStyle.Render(ui.Sanitizef("%s REAL-WORLD EXAMPLES", ui.Icon("💡", "*"))))
	fmt.Println()
...
func printDocsMcp() {
	fmt.Println(ui.SectionStyle.Render(ui.SanitizeString("🤖 MCP SERVER - DETAILED DOCUMENTATION")))
	fmt.Println()
	...
	fmt.Println("  " + ui.Icon("•", "-") + " WAF detection: reports each detection technique as it runs")
	fmt.Println("  " + ui.Icon("•", "-") + " Scanning: streams per-payload results with bypass notifications")
	fmt.Println("  " + ui.Icon("•", "-") + " All tools report progress via MCP progress notifications")
	fmt.Println()
...
func printDocsWorkflow() {
	fmt.Println(ui.SectionStyle.Render(ui.SanitizeString("🔄 RECOMMENDED WORKFLOWS")))
	fmt.Println()
...
func printDocsTemplates() {
	fmt.Println(ui.SectionStyle.Render(ui.SanitizeString("📋 TEMPLATE LIBRARY - DETAILED DOCUMENTATION")))
	fmt.Println()
