...
func TestSentinelErrors_Wrapping(t *testing.T) {
	tests := []struct {
		...
		{"ErrMissingRequired", ErrMissingRequired, "config: missing required field"},
	}
...
func TestSentinelErrors_Distinct(t *testing.T) {
	if errors.Is(ErrInvalidConfig, ErrMissingRequired) {
		t.Error("ErrInvalidConfig and ErrMissingRequired must be distinct")
	}
}
