...
func TestScanner_DetectVulnerability(t *testing.T) {
	...
	tests := []struct {
		...
		{"XPath error in query", true},
		{"XMLError: invalid expression", true},
		{"SimpleXML parsing failed", true},
		{"Normal response", false},
		{"DOMDocument error", true},
	}
...
func TestScanner_VulnerableServer(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Write([]byte("XPath error: invalid expression"))
	}))
	defer server.Close()
